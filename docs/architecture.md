...
2. `POST /api/auth/profile` accepts `{ key, remember }`. Requires the challenge. The key resolves a profile in the challenge’s family, then the challenge is consumed atomically; there is no profile credential. Database stores only hashes of random 256-bit session/device tokens. The authenticated identity is always derived from those cookies, never a requested profile ID.
3. Sessions have a fixed expiry: 12 hours (browser-session cookie) or 30 days (remembered). Both opaque cookies must match. No fingerprinting, sliding “last active” update, heartbeat, or activity timestamps. Direct protected destinations resume a valid session; `/` and `/enter` always show the cottage. Signing in or out performs full navigation to discard client profile state.
4. `POST /api/auth/reauth` accepts `{ adminKey }`, checks the family’s separate administration-key hash and grants 10 minutes of sensitive permissions on this session only. Child profiles cannot reauthenticate as parents.
...
Warm navy, cream, coral, gold and green tokens; centered layouts; four primary destinations without a desktop sidebar. Large controls, visible keyboard focus, descriptive headings, reduced-motion support. The immersive cottage entrance has a brief reduced-motion-aware transition. Home is a truthful shell; future priority cards contain no invented unread counts or activity.
...
- `POST /api/board/prompt` requires fresh `content:manage`; `/settings` requires fresh `family:manage`. Both use the existing administrative verification endpoint. Strict schemas reject submitted identities, unknown fields, empty category selections and invalid times. Settings and custom prompts apply to the next new daily board; an existing board's reveal instant and prompt are fixed.
...
- Parent preview/publish/publication endpoints require fresh `content:manage` permission via existing administrative verification. The preview operates on the parent's supplied file and explicitly allows each player's author view. Publishing revalidates server-side, uses a 1 MB streamed body cap, and requires increasing versions for changed packages. Unpublishing prevents new games; active sessions and completed memories use their snapshots. No player-facing API returns the author's full package.
...
## Cottage entry and family authentication (implemented)

Migration 005 drops profile credential hashes, adds an optional family administration-key hash, clears old sensitive grants and outstanding entry challenges. Existing profile IDs and content survive. The entrance always shows the supplied cottage artwork and a coordinate-aligned accessible door hotspot. A native modal handles the shared password; selecting Mom, Dad or Mia consumes the family challenge and issues the existing opaque session/device cookies without any second credential. Profiles are attribution under shared family trust, not independently authenticated identities.

Sensitive operations retain ten-minute permission grants after separate administration-key verification in a parent session. Missing keys fail closed for those operations only. Provision with `scripts/set-admin-key.ts`; no legacy profile credential is reused. See `docs/ENTRY_FLOW.md` for rollout and review instructions.
